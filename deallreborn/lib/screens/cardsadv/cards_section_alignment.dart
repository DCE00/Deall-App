...
class _CardsSectionState extends State<CardsSectionAlignment>
  ...
  void initState() {
    ...
    // Init cards
    for (cardsCounter = 0; cardsCounter < anuncios.length; cardsCounter++) {
  ...
  Widget build(BuildContext context) {
    return Expanded(
        child: Stack(
          children: <Widget>[
            if (anuncios.length-3 >= cardsSwiped)
            backCard(),
            if (anuncios.length-2 >= cardsSwiped)
            middleCard(),
            if (anuncios.length-1 >= cardsSwiped)
            frontCard(),
